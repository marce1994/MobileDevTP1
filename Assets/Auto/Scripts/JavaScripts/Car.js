...
function SetupGears()
{
	...
	for(var i = 0; i < numberOfGears; i++)
	{
		if(i > 0)
			...
			gearSpeeds[i] = (tempTopSpeed / 4) * AcelPrim;
...
function SetDragZ (valor : float) 
{
 	dragMultiplier.z = valor;
}
function SetAcel (aceleracion : float) 
{
    throttle = aceleracion;
}
function SetGiro (giro : float) 
{
    steer = giro;
}
function SumaGiro (valor : float) 
{
	if(steer + valor <= 1 && steer + valor >= -1)
    	steer += valor;
}
...
//creo que aca es donde se toca la aceleracion
